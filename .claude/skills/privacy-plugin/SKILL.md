...
| J2Commerce stack | `Support\J2CommerceStack`: the installed component decides the active set (display, recording): `com_j2commerce` installed (even if disabled) → `#__j2commerce_*`, else enabled `com_j2store`. Removal, anonymization, retention cleanup and export run on **all** existing sets (`dataSets()`, `forEachDataSet()` / task `dataSetOverride`), so migration copies in `#__j2store_*` are cleaned too |
