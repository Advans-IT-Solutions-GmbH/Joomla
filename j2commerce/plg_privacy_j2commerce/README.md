...
- **J2Commerce 4 / J2Store:** the checkbox is rendered by the template override `default_shipping_payment.php`. See [Template Integration](#template-integration). The override carries the hidden `task`/`option`/`view` inputs, the button id `button-payment-method` that J2Store's own checkout script binds to, and the Joomla form token: J2Store 4.1.8 (security release of 2026-09-15) checks the token on `shipping_payment_method_validate` and answers "Invalid Token" without it. A copy deployed by an earlier plugin version has neither the token nor those inputs. After updating J2Store to 4.1.8, compare such a copy with `overrides/com_j2store/checkout/default_shipping_payment.php` and take over the whole hidden-field and token block at its end (the three hidden inputs `task`, `option`, `view` plus `HTMLHelper::_('form.token')`) as well as the button id, or replace the copy. While consent is required, the guard in `media/plg_privacy_j2commerce/js/consent-validator.js` blocks that button until the checkbox is ticked, because J2Store 4 has no server-side consent check.
...
**Records of an earlier template override:** consent records with the subject `PLG_PRIVACY_J2COMMERCE` were written by earlier site template overrides, partly afterwards when the MyProfile tab was opened (dated with the newest order). They are no evidence of a checkout consent and are never assigned to an order. On installation and update (all such records), in removal requests (the user's records and guest records containing the request or account e-mail address) and in the cleanup task, the e-mail address, IP address and user agent are removed; the record gets the subject `PLG_SYSTEM_J2COMMERCEPRIVACY_CONSENT_SUBJECT_LEGACY`, the neutral legacy body text in the website's default site language and `state = -1` (invalid). Such records are not shown in the Privacy tab and do not count as consent. The installer reports how many records were anonymized.
...
**Result:** • Automatic anonymization of orders outside the retention period
...
Has lifetime licenses → Partial anonymization (keep order e-mail only)
```
