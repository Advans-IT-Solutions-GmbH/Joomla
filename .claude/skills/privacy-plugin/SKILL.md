...
| Legacy consents | Records of the earlier advans template override (`PLG_PRIVACY_J2COMMERCE`, e-mail in the body, partly created afterwards) are **never** assigned to an order: only anonymized (`anonymizeLegacyConsents()`: installer with admin message, removal request incl. guest records with the request/account e-mail, task; batches), subject `PLG_SYSTEM_J2COMMERCEPRIVACY_CONSENT_SUBJECT_LEGACY`, neutral body, not shown or counted in the Privacy tab |
