...
**When:** on J2Commerce 6, when the shopper places the order (`checkout.confirmPayment`, POST) after ticking the checkbox in the shipping & payment step, for the order of this checkout (user state `j2commerce.order_id`) if it was created from the cart the consent was given for. J2Commerce already saves an incomplete order when the confirmation step is rendered (and a new one when the cart changes); those saves record nothing. Exactly one record is written per order: a second request for the same order finds the existing valid record and writes nothing.
...
**Records of an earlier template override:** consent records with the subject `PLG_PRIVACY_J2COMMERCE` (written by earlier site template overrides, with the e-mail address, IP address and user agent in the body and no order reference) are processed on installation and update, in removal requests (for that user) and by the cleanup task. A record is assigned to its order when exactly one order of the same user (guests: the same e-mail address) was created within one hour of the record and that order has no checkout consent yet; it then becomes a regular checkout consent of that order (order number, IP address, user agent; no e-mail address). Otherwise the e-mail address, IP address and user agent are removed and the record stays as evidence; the Privacy tab shows it as an earlier checkout consent without order reference.
